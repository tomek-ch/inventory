...
module.exports = [
    ...
    async function (req, res, next) {
        ...
        if (errors.length) {
            ...
            // console.log(fabric)
            // const checkedFabrics = fabrics.reduce((acc, curr) => ({
            //     ...acc,
            //     [curr._id.toString()]: fabric.find(obj => obj.type === curr._id.toString()),
            // }), {});
            // console.log(checkedFabrics)
            ...
            return res.render('product/product-form',
                { title: 'Add new product', errors, product, styles, categories, fabrics, percentages });
        }

        const newProduct = await new Product(product).save().catch(next);
